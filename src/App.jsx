    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Vaga</li>
          <li>✓ Controle completo de Veiculo</li>
          <li>✓ Controle completo de Ticket</li>
          <li>✓ Controle completo de Tabela Preco</li>
        </ul>
      </div>
...
function PainelVaga({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["numero", "tipo", "disponivel", "andar", "setor", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/vagas", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/vagas/" + editId : apiBaseUrl + "/api/vagas";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["numero", "tipo", "disponivel", "andar", "setor"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/vagas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "Vaga")}</div>
            ...
            <div className="item-field"><b>andar</b><span>{item["andar"] ?? "—"}</span></div>
      ...
      <input type="number" value={form["andar"] ?? ""} onChange={e => setForm({...form, andar: e.target.value})} />
...
function PainelVeiculo({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["placa", "modelo", "marca", "cor", "clienteNome", "clienteTelefone", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["placa", "modelo", "marca", "cor", "clienteNome", "clienteTelefone"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>cor</b><span>{item["cor"] ?? "—"}</span></div>
            <div className="item-field"><b>clienteNome</b><span>{item["clienteNome"] ?? "—"}</span></div>
            <div className="item-field"><b>clienteTelefone</b><span>{item["clienteTelefone"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["cor"] ?? ""} onChange={e => setForm({...form, cor: e.target.value})} />
      ...
      <label className="field-label">clienteTelefone</label>
      <input type="text" maxLength={15} value={form["clienteTelefone"] ?? ""} onChange={e => setForm({...form, clienteTelefone: formatarTelefone(e.target.value)})} />
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
              </div>
...
function PainelTicket({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [vagaList, setVagaList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["vagaId", "veiculoId", "dataEntrada", "dataSaida", "horas", "valorTotal", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/tickets", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    ...
    fetch(apiBaseUrl + "/api/vagas", { headers: authHeaders(token) }).then(r => r.json()).then(setVagaList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/tickets/" + editId : apiBaseUrl + "/api/tickets";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["vagaId", "veiculoId", "dataEntrada", "dataSaida", "horas", "valorTotal", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["vaga", "veiculo"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/tickets/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "Ticket")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/tickets/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["status"] ?? "Ticket")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>vagaId</b><span>{item["vagaId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>horas</b><span>{item["horas"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>vaga</b><span>{item.vagaId ? ((vagaList.find(o => o.id === item.vagaId) || {}).tipo ?? ("#" + item.vagaId)) : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Ticket</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">vagaId</label>
      <input type="number" value={form["vagaId"] ?? ""} onChange={e => setForm({...form, vagaId: e.target.value})} />
      ...
      <select value={form["status"] ?? "ABERTO"} onChange={e => setForm({...form, status: e.target.value})}>
      ...
      <select value={form["vaga"] ?? ""} onChange={e => setForm({...form, vaga: e.target.value})}>
        <option value="">Selecione...</option>
        {(vagaList || []).map(o => (<option key={o.id} value={o.id}>{o.tipo ?? ("#" + o.id)}</option>))}
      </select>
...
function PainelTabelaPreco({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["tipo", "valorHora", "valorDiaria", "valorMensal", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/tabelaprecos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/tabelaprecos/" + editId : apiBaseUrl + "/api/tabelaprecos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["tipo", "valorHora", "valorDiaria", "valorMensal"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/tabelaprecos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">T</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/tabelapreco" + item.id + "/320/200"} alt="TabelaPreco" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["tipo"] ?? "TabelaPreco")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>valorHora</b><span>{item["valorHora"] != null ? Number(item["valorHora"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>valorDiaria</b><span>{item["valorDiaria"] != null ? Number(item["valorDiaria"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>valorMensal</b><span>{item["valorMensal"] != null ? Number(item["valorMensal"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.valorHora)}>Cobrar via Pix</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} TabelaPreco</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">tipo</label>
      <input type="text" value={form["tipo"] ?? ""} onChange={e => setForm({...form, tipo: e.target.value})} />
      <label className="field-label">valorHora</label>
      <input type="number" step="0.01" value={form["valorHora"] ?? ""} onChange={e => setForm({...form, valorHora: e.target.value})} />
      <label className="field-label">valorDiaria</label>
      <input type="number" step="0.01" value={form["valorDiaria"] ?? ""} onChange={e => setForm({...form, valorDiaria: e.target.value})} />
      <label className="field-label">valorMensal</label>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Vaga");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Estacionamento<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Vaga" ? " active" : "")} onClick={() => setAba("Vaga")}><span className="nav-ico">🗂️</span>Vaga</button>
        <button className={"nav-btn" + (aba === "Veiculo" ? " active" : "")} onClick={() => setAba("Veiculo")}><span className="nav-ico">🚗</span>Veiculo</button>
        <button className={"nav-btn" + (aba === "Ticket" ? " active" : "")} onClick={() => setAba("Ticket")}><span className="nav-ico">🗂️</span>Ticket</button>
        <button className={"nav-btn" + (aba === "TabelaPreco" ? " active" : "")} onClick={() => setAba("TabelaPreco")}><span className="nav-ico">🗂️</span>TabelaPreco</button>
      ...
      <main className="main">
        ...
        {aba === "Vaga" && <PainelVaga token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Veiculo" && <PainelVeiculo token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Ticket" && <PainelTicket token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "TabelaPreco" && <PainelTabelaPreco token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
